...
class RolController extends Controller {
  ...
  public function actionBorrar($id) {
    if ($this->loadModel($id)->delete()) {
      Yii::app()->user->setFlash('success', "Su dato ha sido borrado!");
  ...
  public function actionIndex() {
    if (Yii::app()->user->id_usuario == 2) {
      $model = Rol::model()->findAll();
    } else {
      $criteria = new CDbCriteria();
      $criteria->addCondition("id <> :id");
      $criteria->params = array(':id' => 8);
      $model = Rol::model()->findAll($criteria);
    }

    $subMenu = PermisosController::actionGetSubMenu(', Rol', Yii::app()->user->id_usuario);

    $permiso = PermisosController::actionGetPermisos('rol', Yii::app()->user->id_usuario);
